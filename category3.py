...
ads_daily_df = output_groupby_df(ads_daily_df,['SKU', 'SPU', 'Date','Product Type 3'], ['impression','cost','click','conversions','ads value'], 'sum').reset_index()
...
if and_tags or or_tags or exclude_tags:
 ...
 # ---------------------------------------------------------------------汇总数据表开始---------------------------------------------------------------------------------------------------
 # 汇总合并三级去日期数据
 category3_raw_summary_df = output_groupby_df(category3_before_process_df, ['Product Type 3', '日期范围'],
 ...
 # 列格式指定
 category3_summary_column_config = create_dynamic_column_setting(category3_raw_summary_df, ['Product Type 3'],
                                                                 [''],
 ...
 category3_summary_options = st.multiselect(
     ...
     ['Product Type 3', 'impression', 'CPC', 'cost', 'GMV', '神策ROI', 'cost占比', 'click占比', '销量占比', 'gmv占比']
 )
 st.dataframe(category3_raw_summary_df[category3_summary_options].set_index('Product Type 3'),
              column_config=category3_summary_column_config)
 # ---------------------------------------------------------------------汇总数据表结束---------------------------------------------------------------------------------------------------
 # ---------------------------------------------------------------------趋势数据表开始---------------------------------------------------------------------------------------------------
 # 汇总合并三级带日期数据
 category3_daily_df = output_groupby_df(category3_before_process_df, ['Product Type 3', 'Date'],
 ...
 category3_daily_df.sort_values(by=['Product Type 3', 'Date'], ascending=[True, True], inplace=True)
 category3_list_df = output_groupby_df(category3_daily_df, ['Product Type 3'],
 ...
 category3_list_df = add_groupby_sum_columns_to_list_df(category3_daily_df, category3_list_df, ['Product Type 3'],
                                                        'cost', 'cost_sum')
 category3_list_df = add_groupby_sum_columns_to_list_df(category3_daily_df, category3_list_df, ['Product Type 3'],
                                                        'GMV', 'gmv_sum')
 category3_list_df = add_groupby_sum_columns_to_list_df(category3_daily_df, category3_list_df, ['Product Type 3'],
 ...
 # 列格式指定
 category3_list_column_config = create_dynamic_column_setting(category3_list_df, ['Product Type 3', 'Date'],
                                                              [''],
 ...
 category3_trend_options = st.multiselect(
     ...
     ['Product Type 3', 'cost_sum', 'gmv_sum', '神策总ROI', 'ads总ROI', 'impression', 'CPC', 'cost', 'GMV']
 )
 st.dataframe(category3_list_df[category3_trend_options].set_index('Product Type 3'),
 ...
 category3_proportion_df = category3_proportion_df[
     ['Product Type 3', 'Date', 'impression', 'cost', 'click', 'ads value', 'conversions', 'GMV', 'impression占比',
      'cost占比', 'click占比', 'adsvalue占比', 'conversion占比', 'GMV占比', '销量占比']]

 category3_proportion_list_df = output_groupby_df(category3_proportion_df, ['Product Type 3'],
 ...
 category3_proportion_list_df = add_groupby_sum_columns_to_list_df(category3_daily_df, category3_proportion_list_df,
                                                                   ['Product Type 3'], 'cost', 'cost_sum')
 category3_proportion_list_df = add_groupby_sum_columns_to_list_df(category3_daily_df, category3_proportion_list_df,
                                                                   ['Product Type 3'], 'GMV', 'gmv_sum')
 category3_proportion_list_df = add_groupby_sum_columns_to_list_df(category3_daily_df, category3_proportion_list_df,
                                                                   ['Product Type 3'], 'ads value', 'ads_value_sum')
 ...
 category3_proportion_list_df = category3_proportion_list_df[
     ['Product Type 3', 'cost_sum', 'gmv_sum', 'ads_value_sum', '神策总ROI', 'ads总ROI', 'Date', 'impression占比',
      'cost占比', 'click占比', 'adsvalue占比', 'conversion占比', 'GMV占比', '销量占比']]
 # 列格式指定
 ...
 category3_proportion_list_options = st.multiselect(
     ...
     ['Product Type 3', 'cost_sum', 'gmv_sum', 'impression占比', 'cost占比', 'click占比', 'adsvalue占比',
      'conversion占比', 'GMV占比']
 )
 st.dataframe(category3_proportion_list_df[category3_proportion_list_options].set_index('Product Type 3')
 ...
 st.subheader('对比数据')
 unique_category_3 = compare_ads_daily_filtered_date_range_df['Product Type 3'].unique()
 ...
 compare_category3_origin_summary_df = output_groupby_df(compare_category3_before_process_df,
                                                         ...
                                                         'sum').reset_index()
 category3_origin_summary_df = output_groupby_df(category3_before_process_df, ['Product Type 3', '日期范围'],
                                                 ['impression', 'cost', 'click', 'conversions', 'ads value', 'saleuser',
                                                  'sale', 'GMV', 'UV', 'AddtoCart'], 'sum').reset_index()
 if category3_options:
     category3_origin_summary_df = category3_origin_summary_df[
         category3_origin_summary_df['Product Type 3'].isin(category3_options)].drop(columns=['Product Type 3'])
     compare_category3_origin_summary_df = compare_category3_origin_summary_df[
         compare_category3_origin_summary_df['Product Type 3'].isin(category3_options)].drop(columns=['Product Type 3'])
    ...
    # 添加总和列,直接沿用list_df的groupby-list
    category3_list_df = add_groupby_sum_columns_to_list_df(category3_daily_df,category3_list_df ,['Product Type 3'],'cost','cost_sum')
    category3_list_df = add_groupby_sum_columns_to_list_df(category3_daily_df,category3_list_df ,['Product Type 3'],'GMV','gmv_sum')
    category3_list_df = add_groupby_sum_columns_to_list_df(category3_daily_df,category3_list_df ,['Product Type 3'],'ads value','ads_value_sum')
    category3_list_df = add_custom_proportion_to_df(category3_list_df,'gmv_sum','cost_sum','神策总ROI').round(2)
    category3_list_df = add_custom_proportion_to_df(category3_list_df,'ads_value_sum','cost_sum','ads总ROI').round(2)
    # 列格式指定
    category3_list_column_config = create_dynamic_column_setting(category3_list_df,['Product Type 3','Date'],
    ...
    category3_proportion_df = add_custom_proportion_to_df_x100(category3_proportion_df,'GMV','sum_GMV','GMV占比')
    category3_proportion_df = add_custom_proportion_to_df_x100(category3_proportion_df,'sale','sum_sale','销量占比')
    category3_proportion_df = category3_proportion_df[['Product Type 3','Date','impression','cost','click','ads value','conversions','GMV','impression占比','cost占比','click占比','adsvalue占比','conversion占比','GMV占比','销量占比']]
    ...
    category3_proportion_list_df = add_groupby_sum_columns_to_list_df(category3_daily_df,category3_proportion_list_df,['Product Type 3'],'cost','cost_sum')
    category3_proportion_list_df = add_groupby_sum_columns_to_list_df(category3_daily_df,category3_proportion_list_df ,['Product Type 3'],'GMV','gmv_sum')
    category3_proportion_list_df = add_groupby_sum_columns_to_list_df(category3_daily_df,category3_proportion_list_df ,['Product Type 3'],'ads value','ads_value_sum')
    ...
    category3_proportion_list_df = category3_proportion_list_df[['Product Type 3','cost_sum', 'gmv_sum','ads_value_sum','神策总ROI','ads总ROI','Date','impression占比','cost占比','click占比','adsvalue占比','conversion占比','GMV占比','销量占比']]
    # 列格式指定
    category3_proportion_list_column_config = create_dynamic_column_setting(category3_proportion_list_df,['Product Type 3','Date'],
    [''], ['cost_sum', 'gmv_sum','ads_value_sum','神策总ROI','ads总ROI'], [''], [''],0,15)
    st.subheader('占比数据')
    category3_proportion_list_options = st.multiselect(
    '选择三级类目占比数据维度',
    category3_proportion_list_df.columns,
    ['Product Type 3', 'cost_sum', 'gmv_sum','impression占比','cost占比','click占比','adsvalue占比','conversion占比','GMV占比']
    )
    st.dataframe(category3_proportion_list_df[category3_proportion_list_options].set_index('Product Type 3')
                 ,column_config=category3_proportion_list_column_config
                 ,width = 1600, height = 400)
    ...
    compare_category3_before_process_df = pd.merge(compare_ads_daily_filtered_date_range_df,compare_sensor_summary_df[['SKU', 'Date', '日期范围','GMV', 'UV', 'AddtoCart', 'saleuser', 'sale']],
                                           on=['SKU', 'Date','日期范围'], how='left')
    # 汇总合并三级去日期数据
    compare_category3_origin_summary_df = output_groupby_df(compare_category3_before_process_df, ['Product Type 3','日期范围'],
                ['impression','cost','click','conversions','ads value','saleuser','sale','GMV','UV','AddtoCart'], 'sum').reset_index()
    category3_origin_summary_df = output_groupby_df(category3_before_process_df, ['Product Type 3','日期范围'],
                ['impression','cost','click','conversions','ads value','saleuser','sale','GMV','UV','AddtoCart'], 'sum').reset_index()
    if category3_options:
        category3_origin_summary_df = category3_origin_summary_df[category3_origin_summary_df['Product Type 3'].isin(category3_options)].drop(columns=['Product Type 3'])
        compare_category3_origin_summary_df = compare_category3_origin_summary_df[compare_category3_origin_summary_df['Product Type 3'].isin(category3_options)].drop(columns=['Product Type 3'])
